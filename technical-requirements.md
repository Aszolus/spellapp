...
8. Sheet UI should consume derived facts and tracked play state directly; it should not maintain a second independent copy of rules-derived values.
...
## 11. Character Sheet Requirements

1. The character sheet must present the current playable state of the character.
2. Sheet values must be derived from stored build choices, level choices, bundled rules data, and tracked runtime state wherever possible.
3. Every derived value should have source metadata or an explainability path suitable for a details view.
4. Manual overrides must be modeled explicitly and displayed as overrides.
5. Incomplete or invalid builder state must not be presented as a complete playable sheet.
6. Sheet sections should prioritize at-table use: HP, defenses, saves, Perception, skills, actions/strikes, class DCs, spellcasting, resources, conditions, equipment, and carried Bulk.
...
4. UI tests:
   ...
   - sheet facts matching derived builder state,
...
## 16. Release and Update Strategy
...
## 17. Technical Acceptance Criteria (Current MVP)
...
5. The saved character can produce a trusted level-one sheet from derived facts.
6. Spell browsing and details are fully available in airplane mode.
7. CI or local Gradle checks reject builds that violate network isolation or license validation rules.
